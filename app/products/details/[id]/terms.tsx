...
export default function ProductTerms() {
  ...
  if (error || !product) {
    return <View style={styles.centered}><Text style={{ color: Colors.primary, fontSize: 18 }}>{error || 'Product not found.'}</Text></View>;
  }
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 32 }}>
        ...
        <View style={styles.payoutBox}>
          ...
          <View style={styles.payoutRow}>
            <View style={styles.payoutIconBox}>
              <FontAwesome5 name="rupee-sign" size={20} color="#4caf50" style={{ marginRight: 8 }} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.payoutAmount}>₹{payoutAmount}</Text>
              <Text style={styles.payoutDesc}>Earn flat ₹{payoutAmount} on every successful card approval</Text>
            </View>
          </View>
          <View style={styles.payoutRow}>
            <View style={styles.payoutIconBox}>
              <FontAwesome5 name="coins" size={20} color="#ffb300" style={{ marginRight: 8 }} />
            </View>
            <View style={{ flex: 1 }}>
        ...
        <View style={styles.termsSection}>
          <View style={styles.termsHeaderRow}>
            <MaterialIcons name="campaign" size={22} color={Colors.primary} style={{ marginRight: 6 }} />
            <Text style={styles.termsHeader}>You can get payout ONLY if the customer:</Text>
          </View>
          {terms.map((t, i) => (
